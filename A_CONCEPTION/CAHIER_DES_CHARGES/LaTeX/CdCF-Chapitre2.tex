...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}
		...
		\tikzstyle{one} = [fill=colone]
		\tikzstyle{two} = [fill=coltwo]
		\tikzstyle{three} = [fill=colthree]
		\tikzstyle{four} = [fill=colfour]
		\tikzstyle{five} = [fill=colfive]
		\tikzstyle{six} = [fill=colsix]
		\tikzstyle{seven} = [fill=colseven]
		...
		% noeuds supérieurs avec images
		\node[sup,one]   at (  0mm,45mm) {};
		\node[sup,two]   at ( 20mm,45mm) {};
		\node[sup,three] at ( 40mm,45mm) {};
		\node[sup,four]  at ( 60mm,45mm) {};
		\node[sup,five]  at ( 80mm,45mm) {};
		\node[sup,six]   at (100mm,45mm) {};
		\node[sup,seven] at (120mm,45mm) {};
		\node               (Logistician)   at (  0mm,45mm) {\centering\includegraphics[width=16mm]{"Logistician"}};
		\node               (Application)   at ( 20mm,45mm) {\centering};
		\node[oneonthree]   (Laptop)        at ( 40mm,18mm) {\centering\includegraphics[width=16mm]{"Laptop computer"}};
		\node[twoonthree]   (Smartphone)    at ( 40mm,45mm) {\centering\includegraphics[height=16mm]{"Smartphone"}};
		\node[threeonthree] (Satphone)      at ( 40mm,72mm) {\centering\includegraphics[height=16mm]{"Smartphone"}};
		\node[oneonfour,minimum width=10mm] (CompactDisk1) at ( 55mm,15mm) {\centering\includegraphics[width=6mm]{"Compact disk"}};
		\node[oneonfour,minimum width=10mm] (UsbKey1) at ( 65mm,15mm) {\centering\includegraphics[width=8mm]{"USB key"}};
		\node[twoonfour]    (Internet1)     at ( 60mm,35mm) {\centering\includegraphics[width=16mm]{"Internet"}};
		\node[threeonfour]  (PhoneAntenna1) at ( 60mm,55mm) {\centering\includegraphics[height=16mm]{"Phone antenna"}};
		\node[fouronfour]   (Satellite1)    at ( 60mm,75mm) {\centering\includegraphics[width=16mm]{"Satellite"}};
		\node               (Server1)       at ( 80mm,45mm) {\centering\includegraphics[width=16mm]{"Server"}};
		\node[oneonfour,minimum width=10mm] at ( 95mm,15mm) {\centering\includegraphics[width=6mm]{"Compact disk"}};
		\node[oneonfour,minimum width=10mm] at (105mm,15mm) {\centering\includegraphics[width=8mm]{"USB key"}};
		\node[twoonfour]    (Internet2)     at (100mm,35mm) {\centering\includegraphics[width=16mm]{"Internet"}};
		\node[threeonfour]  (PhoneAntenna2) at (100mm,55mm) {\centering\includegraphics[height=16mm]{"Phone antenna"}};
		\node[fouronfour]   (Satellite2)    at (100mm,75mm) {\centering\includegraphics[width=16mm]{"Satellite"}};
		\node               (Server2)       at (120mm,45mm) {\centering\includegraphics[width=16mm]{"Server"}};
		% noeuds inférieurs avec textes
		\node[inf,one]   at (  0mm, 0mm) {\centering\tiny Utilisateur};
		\node[inf,two]   at ( 20mm, 0mm) {\centering\tiny Application};
		\node[inf,three] at ( 40mm, 0mm) {\centering\tiny Matériels};
		\node[inf,four]  at ( 60mm, 0mm) {\centering\tiny Synchronisation};
		\node[inf,five]  at ( 80mm, 0mm) {\centering\tiny Serveur local};
		\node[inf,six]   at (100mm, 0mm) {\centering\tiny Synchronisation};
		\node[inf,seven] at (120mm, 0mm) {\centering\tiny Serveur central};
		% liaisons entre noeuds
		\draw[<->] (Logistician) to             (Application);
		\draw[<->] (Application) to[bend right] (Laptop);
		\draw[<->] (Application) to             (Smartphone);
		\draw[<->] (Application) to[bend left]  (Satphone);
		\draw[<->] (Laptop)      to[bend rigth] (CompactDisk1);
		\draw[<->] (Laptop)      to[bend rigth] (UsbKey1);
		\draw[<->] (Laptop)      to[bend rigth] (Internet1);
		\draw[<->] (Laptop)      to[bend rigth] (PhoneAntenna1);
		\draw[<->] (Laptop)      to[bend rigth] (Satellite1);
