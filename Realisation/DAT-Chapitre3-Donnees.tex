...
%				\ZNoneKey	{Volume}					: NUMBER	\\	% Volume total
			...
			\umlclass[x=70mm,y=100mm,width=60mm]{Provider}{
				\ZPrimaryKey{ItemCode(Id)}				: NUMBER	\\	% Code de l'article
				\ZNoneKey	{Account}					: STRING	\\	% Numéro du compte
				\ZNoneKey	{CommodityTrackingOrDonor}	: NUMBER	\\	% Unité
				\ZNoneKey	{ItemDescription}			: STRING	\\	% Description de l'article
				\ZNoneKey	{Weight}					: NUMBER	\\	% Unité
				\ZNoneKey	{Volume}					: NUMBER		% Prix unitaire
			}{}
		\end{umlpackage}
	\end{tikzpicture}
