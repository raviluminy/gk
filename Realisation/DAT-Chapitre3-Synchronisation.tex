...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}
	    ...
	    % #### CADRE ROUGE :
		% Fond :
		\draw[fill=red!10] (0,0) rectangle (13, 5);
		% Titres :
		\node[titre] at (6.50,0.00) {Mode hors-ligne};
		% Serveurs :
		\node[serveurloc] at (6.00,2.50) {Serveur local};
		\node[serveurcen] at (11.00,2.50) {Serveur central};
		\node[symbole] at (8.50,2.50) {?};
		\node[symbole] at (3.00,2.50) {X};
		% Traits :
		\draw[line width=2pt] (2, 2.5) -- (4, 2.5);
		\draw[line width=2pt] (7.5, 2.5) -- (9.5, 2.5);
		% Utilisateurs :
		\umlactor[x=1.00, y=2.50]{Utilisateur}
		% #### CADRE VERT :
		% Fond :
		\draw[fill=green!10] (0,6) rectangle (13, 11);
		% Titres :
		\node[titre] at (6.50,6.00) {Mode connecté};
		% Serveurs :
		\node[serveurloc] at (6.00,8.50) {Serveur local};
		\node[serveurcen] at (11.00,8.50) {Serveur central};
		\node[symbole] at (8.50, 8.50) {?};
		% Traits :
